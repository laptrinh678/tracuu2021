				...
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Bài viết</label>
					<div class="col-sm-3">
						...
						<select class="form-control" name="post_id" required="">
	                      		<option value="{{$postid->postmodel->id}}">{{$postid->postmodel->post_name}}</option>
	                      		@foreach($post as $val)
				...
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1">Ảnh 1</label>
					<div class="col-sm-9">
						<div id="div_1">
					       ...
					       <input type="file" required="" name="imgpost_new" multiple="true" onchange="previewImg(event);" id="img_file" accept="image/*" value="{{$postid->img}}">
